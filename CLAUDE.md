...
`{name}.preview.png` files are opt-in — not generated by default. The Drawing2 hash is tracked in `.castle/meta.json` (`drawPreviewHashes`) to avoid redundant re-renders.

Set `drawPreviews: true` in `deck.yaml` to enable preview generation:
```yaml
deckId: abc123
drawPreviews: true
```

Or pass `--draw-previews` to `clone` or `serve` to write this setting on first run.
