...
- `docs/simple/drawing.md` - `onDraw()` and `castle.draw.*`
- `docs/simple/input-time.md` - touch input, tilt, `dt`, timers
- `docs/simple/actors.md` - `my`, actor properties, actor methods, actor creation
- `docs/simple/variables.md` - deck and local variables
- `docs/simple/physics.md` - collision polling, physics queries, joints
- `docs/simple/async-events.md` - `castle.co.*` and `castle.events.*`
- `docs/simple/math.md` - math helpers

Use `docs/full/` only when the simple docs are missing something specific, then return to editing immediately.
...
6. Check logs, take a screenshot, then iterate on the game instead of broadening context.
...
- Use `castle.getTouches()` or `castle.getTouch()`.
