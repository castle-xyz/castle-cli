...
function normalizeForComparison(snapshot: any): any {
  ...
  for (const entry of Object.values(snap.library) as any[]) {
    ...
    // Script.code is large and not meaningful to diff in a round-trip test.
    // Script.errors is re-generated by applySnapshot (WASM compiles the script fresh) and may
    // differ from the server's original compile results — normalize away from both sides.
