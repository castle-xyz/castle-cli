...
export async function screenshot(directory: string = '.') {
  ...
  let res: Response;
  try {
    res = await fetch(`http://localhost:${port}/api/screenshot`, { method: 'POST' });
  } catch (e: any) {
  ...
  const filePath = path.join(directory, data.file);
